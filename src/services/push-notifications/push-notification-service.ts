...
class PushNotificationService {
  ...
  private setupListeners(): void {
    ...
    PushNotifications.addListener('registrationError', handlers.onRegistrationError);

    // Show us the notification payload if the app is open on our device
    PushNotifications.addListener('pushNotificationReceived', handlers.onNotificationReceived);

    // Method called when tapping on a notification
    PushNotifications.addListener('pushNotificationActionPerformed', handlers.onNotificationAction);
  }
  ...
  async sendLowBatteryNotification(deviceName: string, batteryLevel: number): Promise<void> {
    await this.notificationTypes.sendLowBatteryNotification(deviceName, batteryLevel);
  }
